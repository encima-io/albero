# Albero
[![Build Status](https://travis-ci.org/encima-io/albero.svg?branch=master)](https://travis-ci.org/github/encima-io/albero)
...
<a name="guarded"></a>
## Guarded property
You might want to put the columns in the guarded property if you are using Laravels guarded property instead of fillable.
...
To use Albero, then you need to use the HasNestedSets trait:
...
* `left`: left index bound (int)
* `right`: right index bound (int)
...
Descendants of deleted nodes will also be deleted and all the `left` and `right`
...
By default in Baum all results are returned sorted by the `left` index column
...
* Check that the bound indexes `left`, `right` are not null, `right` values greater
than `left` and within the bounds of the parent node (if set).
* That there are no duplicates for the `left` and `right` column values.
* As the first check does not actually check root nodes, see if each root has
the `left` and `right` indexes within the bounds of its children.
...
This method will re-index all your `left`, `right` and `depth` column values,
